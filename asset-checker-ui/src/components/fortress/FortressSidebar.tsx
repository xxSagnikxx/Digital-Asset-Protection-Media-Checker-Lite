...
export function FortressSidebar() {
  ...
  async function handleRunScan() {
    const url = "http://127.0.0.1:8080/api/reports";
    try {
      console.log("[FortressSidebar Run Scan] request:start", { url, method: "GET" });
      const reports = await fetchReports();
      console.log("[FortressSidebar Run Scan] request:success", reports);
      window.dispatchEvent(new CustomEvent("reports:refresh"));
      toast.success("Latest matches refreshed.");
    } catch (error) {
      console.error("[FortressSidebar Run Scan] request:exception", error);
      toast.error("Failed to fetch latest reports.");
    }
  }
