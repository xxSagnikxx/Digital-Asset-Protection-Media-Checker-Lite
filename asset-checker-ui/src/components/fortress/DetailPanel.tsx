import { X, Hash, Calendar, Globe, Layers, Image as ImageIcon } from "lucide-react";
...
export function DetailPanel({ row, onClose }: DetailPanelProps) {
  ...
  return (
    <aside className="panel rounded-md flex flex-col animate-fade-in overflow-hidden">
      ...
      <div className="mt-auto p-4 border-t border-hairline grid grid-cols-2 gap-2">
        <button
          onClick={() => console.log("dismiss", row.id)}
          className="rounded-md border border-hairline bg-panel-raised px-3 py-2 text-[12px] text-foreground hover:bg-accent transition-colors"
        >
          False Positive
        </button>
        <button
          onClick={() => console.log("escalate", row.id)}
...
function PixelHeatmap() {
  // Generated overlay of warm splotches — purely decorative
