...
export default function OfficialAssetUpload() {
  ...
  async function handleZipUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const response = await fetch('http://127.0.0.1:8080/api/import-zip', { 
        method: 'POST', 
        body: fd 
      });
      ...
      <div className="h-screen overflow-y-auto">
        ...
        <div className="p-8 space-y-8">
          <form onSubmit={onSubmit} className="panel rounded-md p-6 max-w-xl space-y-4">
            ...
            >
              {isUploading ? "Uploading..." : "Upload Official Asset"}
            </button>
          </form>

          <div className="panel rounded-md p-6 max-w-xl border border-blue-500/30 bg-blue-500/5">
            <h3 className="text-sm font-bold text-foreground mb-4 uppercase tracking-wider">Bulk Import (Zip Folder)</h3>
            <input 
              type="file" 
              accept=".zip" 
              onChange={handleZipUpload} 
              disabled={isBulkUploading}
              className="w-full text-sm text-muted-foreground file:mr-3 file:rounded file:border-0 file:bg-blue-600 file:px-2 file:py-1 file:text-xs file:text-white cursor-pointer"
            />
            {isBulkUploading && <p className="mt-2 text-xs text-blue-400 animate-pulse">Extracting and indexing assets...</p>}
          </div>
        </div>
      </div>
    </AppShell>
