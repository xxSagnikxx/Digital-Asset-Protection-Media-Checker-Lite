...
export default function Dashboard() {
    ...
    useEffect(() => {
        ...
        const poller = window.setInterval(async () => {
            try {
                ...
                const previews = nextRows.filter((row) => !baselineIds.has(row.id)).map((row) => row.suspect.thumb).filter(Boolean);
                setLiveFeed((current) => {
    ...
    async function handleStartDeepScan() {
        ...
        if (!value) { toast.error("Paste a URL before starting deep scan."); return; }
        setIsHunting(true);
        setHuntSource(value);
        setLiveFeed([]);
        ...
        try {
            ...
            const response = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ url: value }) });
            ...
            if (result.matches_found > 0) {
                toast.error(`High-priority alert: ${result.matches_found} potential match(es) found.`, { duration: 7000 });
    ...
    const confidenceCounts = useMemo<Record<MatchConfidence, number>>(() => ({ high: rows.filter((item) => item.level === "high").length, medium: rows.filter((item) => item.level === "medium").length, low: rows.filter((item) => item.level === "low").length }), [rows]);
    ...
    const heatmap = useMemo(() => {
        ...
        return Array.from({ length: 7 }, (_, dayIdx) => Array.from({ length: 24 }, (_, hourIdx) => source[(dayIdx * 24 + hourIdx) % source.length]));
    }, [rows]);
            ...
            <div className="h-screen overflow-y-auto">
                <header className="flex items-center justify-between px-8 py-5 border-b border-hairline">
                    <div>
                        ...
                        <h1 className="font-display text-3xl text-foreground leading-tight mt-0.5">Asset Integrity Dashboard</h1>
                    </div>
                    <div className="hidden md:flex items-center gap-3 text-[11px] text-muted-foreground">
                        ...
                        <span className="inline-flex items-center gap-1.5"><span className="h-1.5 w-1.5 rounded-full bg-forest" /> Systems nominal</span>
                    </div>
                </header>
                <section className="px-8 pt-6 pb-4 grid grid-cols-1 lg:grid-cols-4 gap-4">
                    <Widget label="Protected Assets" actions={<ShieldCheck className="h-3.5 w-3.5 text-forest" strokeWidth={1.6} />}>
                        <div className="flex items-end justify-between gap-4">
                            <div>
                                <p className="font-display text-4xl font-medium text-foreground tabular leading-none">{rows.length}</p>
                                <p className="text-[11px] text-muted-foreground mt-1.5 inline-flex items-center gap-1"><ArrowUpRight className="h-3 w-3 text-forest" /> Live from API</p>
                            </div>
                            <div className="h-16 w-[60%]"><Sparkline data={trendData} /></div>
                        </div>
                    ...
                    <Widget label="Risk Profile" hint="Last 7 days" actions={<Flame className="h-3.5 w-3.5 text-terracotta" strokeWidth={1.6} />}>
                        <div className="flex items-start justify-between gap-4 mb-3">
                            ...
                            <div className="text-right">
                                ...
                                <p className="font-display text-3xl font-medium text-terracotta tabular leading-none mt-1">{confidenceCounts.high}</p>
                                <p className="text-[10px] text-muted-foreground mt-1 tabular">of {totalMatches} total</p>
                            </div>
                        </div>
                        <Heatmap data={heatmap} />
                    </Widget>
                    <Widget label="Scan Operations" hint="Today vs Average" actions={<ScanLine className="h-3.5 w-3.5 text-copper" strokeWidth={1.6} />}>
                        <div className="flex items-center justify-between gap-4">
                            <div>
                                <p className="font-display text-4xl font-medium text-foreground tabular leading-none">{rows.length}</p>
                                <p className="text-[11px] text-muted-foreground mt-1.5 inline-flex items-center gap-1"><ArrowUpRight className="h-3 w-3 text-forest" /> Updated from server</p>
                        ...
                        <div className="mt-3 pt-3 border-t border-hairline flex items-center justify-between text-[10px] text-muted-foreground">
                            ...
                            <span className="font-mono inline-flex items-center gap-1 text-foreground"><ArrowDownRight className="h-3 w-3 text-terracotta" /> API backed</span>
                        </div>
                    </Widget>
                    <Widget label="Active URL Hunt" hint="Paste URL and launch" actions={<ScanLine className="h-3.5 w-3.5 text-copper" strokeWidth={1.6} />}>
                        <div className="space-y-3">
                            <div className="flex gap-2">
                                <input type="url" value={huntUrl} onChange={(event) => setHuntUrl(event.target.value)} placeholder="https://example.com/gallery" className="flex-1 rounded-md border border-hairline bg-panel-raised px-3 py-2 text-[12px] text-foreground placeholder:text-muted-foreground" />
                                <button type="button" onClick={handleStartDeepScan} disabled={isHunting} className="rounded-md brushed-copper px-3 py-2 text-[12px] font-medium text-[hsl(38_75%_88%)] ring-1 ring-[hsl(22_70%_55%/0.45)] disabled:opacity-60">{isHunting ? "Scanning..." : "Start Deep Scan"}</button>
                            </div>
                ...
                <section className="px-8 pb-8 grid grid-cols-1 xl:grid-cols-[1fr_360px] gap-4">
                    {isLoading ? ( <div className="panel rounded-md p-6 text-sm text-muted-foreground">Scanning...</div> ) : ( <FeedTable rows={rows} selectedId={selected?.id ?? null} onSelect={setSelected} /> )}
...
function Mini({ label, value, tone }: { label: string; value: string; tone?: "terracotta" }) {
    return ( <div><p className="text-muted-foreground/80">{label}</p><p className={`font-data text-sm tabular mt-0.5 ${tone === "terracotta" ? "text-terracotta" : "text-foreground"}`}>{value}</p></div> );
}
function RiskRow({ color, label, value }: { color: "terracotta" | "copper" | "forest"; label: string; value: number; }) {
    const map = { terracotta: "bg-terracotta", copper: "bg-copper", forest: "bg-forest" } as const;
    return ( <div className="flex items-center gap-2"><span className={`h-1.5 w-1.5 rounded-full ${map[color]}`} /><span className="text-[11px] text-muted-foreground w-14">{label}</span><span className="font-data text-[13px] tabular text-foreground">{value}</span></div> );
}