...
export default function MatchReports() {
    ...
    const handleStatusUpdate = useCallback(
        async (matchId: string, newStatus: MatchStatus) => {
            setRows((prev) => prev.map((r) => (r.id === matchId ? { ...r, statusUpdating: true } : r)));
            try {
                await updateMatchStatus(matchId, newStatus);
                setRows((prev) => prev.map((r) => r.id === matchId ? { ...r, status: newStatus, statusUpdating: false } : r));
                setSelectedId((prev) => {
                ...
                setRows((prev) => prev.map((r) => (r.id === matchId ? { ...r, statusUpdating: false } : r)));
    ...
    const displayRows = useMemo(() => (viewMode === "pending" ? rows.filter((r) => r.status === "pending") : rows), [rows, viewMode]);
            ...
            <div className="h-screen overflow-y-auto">
                <header className="px-8 py-5 border-b border-hairline flex items-end justify-between gap-4">
                    ...
                    {!isLoading && !error && (
                        <div className="flex items-center gap-1 rounded-md border border-hairline bg-panel-raised p-1 mb-0.5">
                            <TabPill label="Pending" count={pendingCount} active={viewMode === "pending"} onClick={() => setViewMode("pending")} />
                            <TabPill label="All Reports" count={totalCount} active={viewMode === "all"} onClick={() => setViewMode("all")} />
                        </div>
                    )}
                </header>
                <section className="px-8 py-6 grid grid-cols-1 xl:grid-cols-[1fr_360px] gap-4">
                    {isLoading ? (
                        ...
                        <>
                            <FeedTable rows={displayRows} selectedId={selectedId} onSelect={(row) => setSelectedId(row.id)} onStatusUpdate={handleStatusUpdate} />
                            <DetailPanel row={selectedRow} onClose={() => setSelectedId(null)} onStatusUpdate={handleStatusUpdate} />
                        </>
                    )}
                </section>
            </div>
...
function TabPill({ label, count, active, onClick }: { label: string; count: number; active: boolean; onClick: () => void; }) {
    return (
        <button onClick={onClick} className={`flex items-center gap-1.5 rounded px-3 py-1.5 text-[11px] font-medium uppercase tracking-wider transition-colors ${active ? "bg-copper/20 text-copper border border-copper/30" : "text-muted-foreground hover:text-foreground hover:bg-accent/30 border border-transparent"}`}>
            {label}
            <span className={`rounded-full px-1.5 py-0.5 text-[9px] font-mono ${active ? "bg-copper/30 text-copper" : "bg-accent/60 text-muted-foreground"}`}>{count}</span>
        </button>
    );
}
    ...
    return (
        <div className="panel rounded-md p-8 flex flex-col items-center justify-center gap-3 min-h-[240px]">
            <div className="flex gap-1.5">
                {[0, 1, 2].map((i) => (
                    <span key={i} className="h-2 w-2 rounded-full bg-copper/60 animate-bounce" style={{ animationDelay: `${i * 0.15}s` }} />
                ))}
            </div>
...
function ToastNotification({ toast }: { toast: Toast }) {
    ...
    return (
        <div className={`pointer-events-auto flex items-center gap-3 rounded-md border px-4 py-3 text-[12px] shadow-[var(--shadow-elevated)] animate-fade-in backdrop-blur-sm ${isSuccess ? "border-forest/30 bg-forest/10 text-forest" : "border-terracotta/30 bg-terracotta/10 text-terracotta"}`}>
            {isSuccess ? <CheckCircle2 className="h-4 w-4 shrink-0" /> : <AlertTriangle className="h-4 w-4 shrink-0" />}
